...
export default function ApplicationBox({needCoverLetter, onClose, isOpen, jobId, jobName, companyId}) {
  ...
  const submitHandler = async(e) => {
    ...
    .then(async val => {
      if(val.code == 0) {
        ...
        handleClick({message:"Already submitted an application!", type:"error"})
        setLoading(false)
        await new Promise((res) => setTimeout(res, 2000));
      }
    })
  ...
  return (
    <div>
      <Dialog open={isOpen} onClose={onClose}>
        ...
        <DialogActions>
          <Button disabled={loading} onClick={submitHandler}>Submit</Button>
