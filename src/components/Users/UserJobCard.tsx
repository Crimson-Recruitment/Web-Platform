...
dialog:any;
more:any }) {
  const { title, description, timestamp, benefits, maxSalary, minSalary, location, dialog, more } = props;
  ...
  <div>
     <Card sx={{ maxWidth: "100%", margin: '20px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
      ...
      <CardActions>
        <Button size="small" onClick={dialog} sx={{ color: '#2196f3' }}>Apply Now</Button>
        <Button size="small" onClick={more} sx={{ color: '#757575' }}>Learn More</Button>
      </CardActions>
    </Card>
  </div>
  );
