...
const MeetingMenu = () => {
	...
	return (
		<div className="bg-white border py-3 rounded absolute top-[100%] right-[50%] flex flex-col text-gray-500">
			<button onClick={onEditMeetingButtonClicked} className="flex flex-row  hover:bg-gray-100 text-gray-500 px-3">Edit</button>
			<button onClick={onRescheduleMeetingButtonClicked} className="flex flex-row  hover:bg-gray-100 text-gray-500 px-3">Reschedule</button>
		</div>
	);
}
		...
		<div className="flex flex-col border shadow rounded-lg p-2">
			<div className="flex flex-row p-3 items-center gap-2">
				...
				<div className="flex flex-row gap-2 ml-auto items-center">
					...
					<button onClick={toggleMeetingMenuVisible} className="bg-gray-200 aspect-square grid place-items-center rounded-full relative">
				...
				{ meeting.isOnline ?
					(
						<div className="flex flex-row gap-2 items-center">
							...
							<button onClick={onMeetingLinkClicked} className="text-blue-500 underline text-sm">{meeting.meetingLink}</button>
						</div>
					) 
						:
					(
						<div className="flex flex-row gap-2 items-center">
							...
							<button onClick={onMeetingLocationClicked} className="text-blue-500 underline text-sm">{meeting.meetingLocation}</button>
						</div>
					)
				}
			</div>
			<div className="flex flex-col border-t p-3">
				...
				<div className="flex flex-col text-sm">
					<div className={ readMore ? "flex flex-col h-full overflow-hidden" : "flex flex-col h-12 overflow-hidden"}>
						<span className="text-xs text-gray-400">{meeting.otherDetails}</span>
					</div>
					<button onClick={toggleReadMore} className="text-blue-700 font-bold text-sm w-fit p-0 focus:outline-none">Read More</button>
				</div>
			</div>
			<div className="flex flex-row p-3 gap-2 items-center border-t">
